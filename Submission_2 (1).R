...
bike.model <- train(form=count~season+holiday+atemp+humidity+hour,
...
my_submission$count[my_submission$count < 0] <- 0

write.table(my_submission, file = "Bike10.csv", sep = ",",row.names=FALSE, col.names=c("datetime", "count"))
